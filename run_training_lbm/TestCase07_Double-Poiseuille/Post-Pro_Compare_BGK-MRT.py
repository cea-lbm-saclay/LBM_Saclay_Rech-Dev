...
x_list=np.array([])
u_y = np.array([])
with open('Profil_Double-Poiseuille_BGK.csv', newline='') as csvfile:
...
with open('Profil_Double-Poiseuille_MRT.csv', newline='') as csvfile:
    ...
    for row in reader:
        if row[0]!= 'laplaphi':
            ...
            u_y2 = np.append(u_y2, float(row[15]))
...
l3, = ax.plot(u_y2,x_list, linestyle= 'None',marker = 'o', markerfacecolor = 'red', markeredgecolor='red', markevery= 4)
l1, = ax.plot(u_th,x_list, color = 'black', linewidth=3.0)
ax.legend( (l1,l2,l3), ('analytique','BGK','MRT'), loc= 'upper right', shadow = True)
