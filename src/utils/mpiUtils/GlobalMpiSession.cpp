...
GlobalMpiSession::GlobalMpiSession(int* argc, char*** argv, std::ostream* out)
{
    ...
    TEST_FOR_EXCEPTION_PRINT(
        mpierr != 0, std::runtime_error, "Error code=" << mpierr << " detected in GlobalMpiSession::GlobalMpiSession(argc,argv)", out);
    ...
    TEST_FOR_EXCEPTION_PRINT(
        mpierr != 0, std::runtime_error, "Error code=" << mpierr << " detected in MPI_Get_processor_name()", out);
...
#ifndef EULER_KOKKOS_SUPPRESS_PROC_STARTUP_BANNER
...
#endif // EULER_KOKKOS_SUPPRESS_PROC_STARTUP_BANNER
