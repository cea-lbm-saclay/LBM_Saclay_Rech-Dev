#ifndef LBMSCHEME_NS_2AC_C_H_
#define LBMSCHEME_NS_2AC_C_H_
...
struct LBMScheme : public LBMSchemeBase<dim, npop> {
  ...
  void setup_collider(EquationTag1 tag, const IVect<dim> &IJK,
    ...
    const real_t G_st = Model.S_st012(tag, lbmState);

    // AJOUTS ALAIN pour counter terms phi2 et phi (ATTENTION pas de tag en
    // argument)
    ...
    RVect<dim> G123_ct;
    G123_ct[IX] = (2.0 / 3.0) * G1_ct[IX] - (1.0 / 3.0) * G2_ct[IX] -
                  (1.0 / 3.0) * G3_ct[IX];
    G123_ct[IY] = (2.0 / 3.0) * G1_ct[IY] - (1.0 / 3.0) * G2_ct[IY] -
                  (1.0 / 3.0) * G3_ct[IY];
    G123_ct[IZ] = (2.0 / 3.0) * G1_ct[IZ] - (1.0 / 3.0) * G2_ct[IZ] -
                  (1.0 / 3.0) * G3_ct[IZ];
    // FIN AJOUTS ALAIN
    ...
    collider.S0[0] =
        w[0] * dt *
        (G_st + G_dw + staudx * Base::compute_scal(0, G123_ct)); // MODIF ALAIN
    ...
    for (int ipop = 1; ipop < npop; ++ipop) {
      ...
      collider.S0[ipop] =
          w[ipop] * dt *
          (G_st + G_dw +
           staudx * Base::compute_scal(ipop, G123_ct)); // MODIF ALAIN
      collider.feq[ipop] =
          w[ipop] * (M2 + Model.PF_advec * dx / dt *
                              Base::compute_scal(ipop, M1) / cs2) -
  ...
  void setup_collider(EquationTag4 tag, const IVect<dim> &IJK,
    ...
    const real_t G2_st = Model.S2_st(lbmState); // Term proportional to lambda
    ...
    if (dim == 2) {
      G123_ct[IX] = (2.0 / 3.0) * G2_ct[IX] - (1.0 / 3.0) * G1_ct[IX] -
                    (1.0 / 3.0) * G3_ct[IX];
      G123_ct[IY] = (2.0 / 3.0) * G2_ct[IY] - (1.0 / 3.0) * G1_ct[IY] -
                    (1.0 / 3.0) * G3_ct[IY];
    } else if (dim == 3) {
      G123_ct[IX] = (2.0 / 3.0) * G2_ct[IX] - (1.0 / 3.0) * G1_ct[IX] -
                    (1.0 / 3.0) * G3_ct[IX];
      G123_ct[IY] = (2.0 / 3.0) * G2_ct[IY] - (1.0 / 3.0) * G1_ct[IY] -
                    (1.0 / 3.0) * G3_ct[IY];
      G123_ct[IZ] = (2.0 / 3.0) * G2_ct[IZ] - (1.0 / 3.0) * G1_ct[IZ] -
                    (1.0 / 3.0) * G3_ct[IZ];
    }
    ...
    // ipop = 0
    collider.f[0] = Base::get_f_val(tag, IJK, 0);
    collider.S0[0] = w[0] * dt *
                     (G2_st + G2_dw +
                      staudx * Base::compute_scal(0, G123_ct)); // MODIF ALAIN
    collider.feq[0] = M0 - (1 - w[0]) * M2 - 0.5 * collider.S0[0];
    // ipop > 0
    for (int ipop = 1; ipop < npop; ++ipop) {
      collider.f[ipop] = this->get_f_val(tag, IJK, ipop);
      collider.S0[ipop] =
          w[ipop] * dt *
          (G2_st + G2_dw + staudx * Base::compute_scal(ipop, G123_ct));
      collider.feq[ipop] =
          w[ipop] * (M2 + dx / dt * Base::compute_scal(ipop, M1) / cs2) -
          0.5 * (collider.S0[ipop]);
    }
  } // end of setup_collider for phase field equation
  ...
  void setup_collider(EquationTag2 tag, const IVect<dim> &IJK,
    ...
    if (dim == 2) {
      ...
      for (int ipop = 0; ipop < npop; ++ipop) {
        real_t scalUC =
            dx / dt * Base::compute_scal(ipop, lbmState[IU], lbmState[IV]);
        GAMMA[ipop] =
      ...
      for (int ipop = 0; ipop < npop; ++ipop) {
        real_t scalUC =
            dx / dt *
            Base::compute_scal(ipop, lbmState[IU], lbmState[IV], lbmState[IW]);
        GAMMA[ipop] =
            scalUC / cs2 + 0.5 * SQR(scalUC) / SQR(cs2) - 0.5 * scalUU / cs2;
        real_t feqbar =
            w[ipop] * (lbmState[IP] / (cs2 * lbmState[ID]) + GAMMA[ipop]);
    ...
    // RVect2 ForceTS = Model.force_TS(lbmState);
    //  AJOUTS ALAIN
    ...
    real_t coeffV =
        -(nu * (Model.rho1 - Model.rho0)) / (cs2 * dt * collider.tau);
    ...
    if (dim == 2) {
      ...
      for (int alpha = 0; alpha < npop; ++alpha) {
        ForceV[IX] += E[alpha][IX] * E[alpha][IX] * lbmState[IDPHIDX] *
                          (collider.f[alpha] - collider.feq[alpha]) +
                      E[alpha][IX] * E[alpha][IY] * lbmState[IDPHIDY] *
                          (collider.f[alpha] - collider.feq[alpha]);
        ForceV[IY] += E[alpha][IY] * E[alpha][IX] * lbmState[IDPHIDX] *
                          (collider.f[alpha] - collider.feq[alpha]) +
                      E[alpha][IY] * E[alpha][IY] * lbmState[IDPHIDY] *
                          (collider.f[alpha] - collider.feq[alpha]);
      }
      ...
      ForceTot[IX] = ForceG[IX] + ForceP[IX] + ForceTS1[IX] + ForceTS2[IX] +
                     ForceTS3[IX] + ForceV[IX];
      this->set_lbm_val(IJK, IFX, ForceTot[IX]);
      ForceTot[IY] = ForceG[IY] + ForceP[IY] + ForceTS1[IY] + ForceTS2[IY] +
                     ForceTS3[IY] + ForceV[IY];
    ...
    if (dim == 3) {
      ...
      ForceTot[IY] = ForceG[IY] + ForceP[IY] + ForceTS1[IY] + ForceTS2[IY] +
                     ForceTS3[IY] + ForceV[IY];
      this->set_lbm_val(IJK, IFY, ForceTot[IY]);
      ForceTot[IZ] = ForceG[IZ] + ForceP[IZ] + ForceTS1[IZ] + ForceTS2[IZ] +
                     ForceTS3[IZ] + ForceV[IZ];
  ...
  void setup_collider(EquationTag2 tag, const IVect<dim> &IJK,
    ...
    FState GAMMA;
    // MODIFS ALAIN
    // RVect2 gradrho = Model.grad_rho(lbmState);
    RVect<dim> gradrho;
    ...
    RVect<dim> ForceTS2 = Model.force_TS2<dim>(lbmState); // AJOUTS ALAIN
    RVect<dim> ForceTS3 = Model.force_TS3<dim>(lbmState); // AJOUTS ALAIN
    //[[maybe_unused]] RVect2 ForceBck = Model.force_Beckermann(lbmState) ;
    // RVect2 ForceP = Model.force_P2(lbmState);
    ...
    // Calcul de la source
    // const real_t nu = Model.nu0*Model.nu1/( ((1.0-lbmState[IPHI]) *
    // Model.nu1) + ((lbmState[IPHI]) * Model.nu0));
    const real_t nu = lbmState[IPHI3] * Model.nu0 + lbmState[IPHI] * Model.nu1 +
                      lbmState[IPHI2] * Model.nu2;
    ...
    if (dim == 2) {
      ...
      for (int beta = 0; beta < npop; ++beta) {
        ForceV[IX] +=
            E[beta][IX] * E[beta][IX] * gradrho[IX] * MinvSMg_geq[beta] +
            E[beta][IX] * E[beta][IY] * gradrho[IY] * MinvSMg_geq[beta];
        ForceV[IY] +=
            E[beta][IY] * E[beta][IX] * gradrho[IX] * MinvSMg_geq[beta] +
            E[beta][IY] * E[beta][IY] * gradrho[IY] * MinvSMg_geq[beta];
      }
      ...
      for (int beta = 0; beta < npop; ++beta) {
        ForceV[IX] +=
            E[beta][IX] * E[beta][IX] * gradrho[IX] * MinvSMg_geq[beta] +
            E[beta][IX] * E[beta][IY] * gradrho[IY] * MinvSMg_geq[beta] +
            E[beta][IX] * E[beta][IZ] * gradrho[IZ] * MinvSMg_geq[beta];
        ForceV[IY] +=
            E[beta][IY] * E[beta][IX] * gradrho[IX] * MinvSMg_geq[beta] +
            E[beta][IY] * E[beta][IY] * gradrho[IY] * MinvSMg_geq[beta] +
            E[beta][IY] * E[beta][IZ] * gradrho[IZ] * MinvSMg_geq[beta];
        ForceV[IZ] +=
            E[beta][IZ] * E[beta][IX] * gradrho[IX] * MinvSMg_geq[beta] +
            E[beta][IZ] * E[beta][IY] * gradrho[IY] * MinvSMg_geq[beta] +
            E[beta][IZ] * E[beta][IZ] * gradrho[IZ] * MinvSMg_geq[beta];
      }
  ...
  void make_boundary(const IVect<dim> &IJK, int faceId) const {
    ...
    if (Base::params.boundary_types[BOUNDARY_EQUATION_4][faceId] ==
      ...
      real_t boundary_value =
          Base::params.boundary_values[BOUNDARY_PHASE_FIELD][faceId];
      for (int ipop = 0; ipop < npop; ++ipop)
        this->compute_boundary_antibounceback(tagPHI2, faceId, IJK, ipop,
                                              boundary_value);
    ...
    if (Base::params.boundary_types[BOUNDARY_EQUATION_2][faceId] ==
      ...
      for (int ipop = 0; ipop < npop; ++ipop) {
        value = dx / dt *
                this->compute_scal_Ebar(ipop, boundary_vx, boundary_vy) / cs2 *
                Model.rho0;
  ...
  void init_macro(IVect<dim> IJK, RANDOM_POOL::generator_type rand_gen) const {
    ...
    if (Model.initType == PHASE_FIELD_INIT_VERTICAL)
      ...
      real_t xCmin, xCmax, yCmin, yCmax, d1, d2, d3, d4, d5;
      xCmin = Model.xmin_Container;
      xCmax = Model.xmax_Container;
      yCmin = Model.ymin_Container;
      yCmax = Model.ymax_Container;
      //~ real_t Epaiss = Model.Width_Container;
      //~ real_t Delta_x = 3.*Model.dx ;
      ...
      // Conditions sur phi
      phi =
          (Model.phi0(d1) - Model.phi0(d2)) * (Model.phi0(d3) - Model.phi0(d4));
      phi2 = Model.phi0(d5);
      ...
      if (dim == 2) {
        ...
        xphi2 =
            ...
            Model.heightPhi2 - y;
        // xc    =
        // Model.ampl*sin(Model.n_wave*2.0*3.14/L*x+3.14/2.0)+Model.heightPhi2 -
        // y;
        phi2 = Model.phi0(xphi2);
        phi -= phi2;
      } else if (dim == 3) {
        // real_t L = Model.wave_length;
        // real_t d1, d2;
        // d1 = z - Model.ampl * (cos(Model.n_wave * x *2.0*3.14 / L) +
        // cos(Model.n_wave * y *2.0*3.14 / L))-Model.height; d2 = Model.z0-z;
        // phi   = Model.phi0(d1) ;
        // phi2   = Model.phi0(d2) ;
        ...
        // d0 = z - Model.height ;
        // //d1 = Model.R  - sqrt(SQR(x - Model.xC ) + SQR(y - Model.yC ) +
        // SQR(z - Model.zC ))   ; if (z<Model.height) phi = Model.phi0(d0) ;
        // //else if (y > Model.height) phi = Model.phi0(d1 ) ;
        // xphi2 = z-Model.ampl * (cos(Model.n_wave * x *2.0*3.14 / L) +
        // cos(Model.n_wave * y *2.0*3.14 / L))-Model.heightPhi2;
        // //xc    =
        // Model.ampl*sin(Model.n_wave*2.0*3.14/L*x+3.14/2.0)+Model.heightPhi2 -
        // y; phi2  = Model.phi0(xphi2) ;
        // //phi  -= phi2;
    ...
    } else if (Model.initType == THREEPHASES_RAYLEIGH_TAYLOR_3D) {
      if (dim == 3) {
        // real_t L = Model.wave_length;
        // real_t d0;
        // d0 = z - Model.height ;
        // //d1 = Model.R  - sqrt(SQR(x - Model.xC ) + SQR(y - Model.yC ) +
        // SQR(z - Model.zC ))   ; phi = Model.phi0(d0) ;
        // //else if (y > Model.height) phi = Model.phi0(d1 ) ;
        // xphi2 = z-Model.ampl * (cos(Model.n_wave * x *2.0*3.14 / L) +
        // cos(Model.n_wave * y *2.0*3.14 / L))-Model.heightPhi2;
        // //xc =  z-Model.ampl * (cos(Model.n_wave * x *2.0*3.14 / L) +
        // cos(Model.n_wave * y *2.0*3.14 / L))-Model.heightPhi2;
        // //xc    =
        // Model.ampl*sin(Model.n_wave*2.0*3.14/L*x+3.14/2.0)+Model.heightPhi2 -
        // y; phi2  = Model.phi0(xphi2) ;
        ...
        real_t a1 = z - Model.height;
        real_t b = z -
                   Model.ampl * (cos(Model.n_wave * x * 2.0 * 3.14 / L) +
                                 cos(Model.n_wave * y * 2.0 * 3.14 / L)) -
                   Model.heightPhi2;
        real_t d1 = fmax(a1, b);
        // real_t a2 = -(z-Model.ampl * (cos(Model.n_wave * x *2.0*3.14 / L) +
        // cos(Model.n_wave * y *2.0*3.14 / L))-Model.heightPhi2); real_t d2 =
        // fmax(a2,b);
        phi = Model.phi2(-a1);
        phi2 = Model.phi0(-d1);
      }
      ...
      xphi3 = FMIN(Model.width - sqrt(SQR(x - Model.x0)),
                   Model.height - sqrt(SQR(y - Model.y0)));
      phi2 = Model.phi0(xphi3);
    } else if (Model.initType == PHASE_FIELD_INIT_SHRF_VORTEX) {
      //~ real_t vx, vy;
      real_t d1, d2;
      //~ const real_t pi = M_PI;
      //~ vx = - pi*Model.U0 * cos(pi*((x/Model.L0)-0.5)) *
      //sin(pi*((y/Model.L0)-0.5)); ~ vy =   pi*Model.U0 *
      //sin(pi*((x/Model.L0)-0.5)) * cos(pi*((y/Model.L0)-0.5));
      d1 = Model.R - sqrt(SQR(x - Model.xC) + SQR(y - Model.yC));
      d2 = Model.R1 - sqrt(SQR(x - Model.xC1) + SQR(y - Model.yC1));
      phi = Model.phi0(d1);
      phi2 = Model.phi0(d2);
    ...
    mu = Model.mu012(c, phi3, phi, phi2);
    // Save
  ...
  void update_macro(IVect<dim> IJK) const {
    ...
    [[maybe_unused]] const real_t rhoprev = lbmStatePrev[ID];
    ...
    //
    const real_t G2_st = Model.S2_st(lbmStatePrev); // AJOUT ALAIN
    ...
    const real_t mu = Model.mu012(c, phi3, phi, phi2);
    // MODIF ALAIN rho mis en commentaires et remplacé
    // const real_t rho = Model.interp_rho(phi,c);
    // const real_t rho =  phi3*Model.rho0 + phi*Model.rho1 + phi2*Model.rho2 ;
    // // AJOUT ALAIN
    const real_t rho = phi3 * Model.rho0c(c) + phi * Model.rho1c(c) +
                       phi2 * Model.rho2c(c); // AJOUT ALAIN
    ...
    const real_t P =
        moment_P * rho * cs2; // Mise A Jour pression avec rho calculé au-dessus
