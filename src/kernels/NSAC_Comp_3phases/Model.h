...
//
// ================================================
namespace NSAC_comp_3phases_2donly {
struct ModelParams {
  ...
  void showParams() {
    if (myRank == 0) {
      ...
      std::cout << "Capillaire = " << SQRT(L * (-gz)) * rho1 * nu1 / Sigma12
                << std::endl;
    }
  };
  ...
  ModelParams(const ConfigMap &configMap, LBMParams params) {
    ...
    GammaT = 3.0 / ((1.0 / Gamma1) + (1.0 / Gamma2) +
                    (1.0 / Gamma3)); // Eq below Eq. (16)
    ...
    c2_co =
        configMap.getFloat("params_composition", "c2_co", 0.0); // AJOUTS ALAIN
    ...
    c2_inf =
        configMap.getFloat("params_composition", "c2_inf", 0.0); // AJOUTS ALAIN
    ...
    L0 = configMap.getFloat(
        "init", "L0",
        0.0); // Largeur du domaine (info redondante avec xmin et xmax)
    R = configMap.getFloat("init", "rayon", 10.0);
    xC = configMap.getFloat("init", "Xcentre", 0.0);
    yC = configMap.getFloat("init", "Ycentre", 0.0);
    zC = configMap.getFloat("init", "Zcentre", height * 1.5);
    ...
    else if (initTypeStr == "rayleigh_taylor_threephases_3d")
      initType = THREEPHASES_RAYLEIGH_TAYLOR_3D;
  ...
  real_t Gamma1, Gamma2, Gamma3,
      GammaT; // MODIFS ALAIN Parameters for surface tensions of 3 phases
  real_t PF_advec;
  real_t gamma, counter_term;
  real_t D0, D1, D2;
  real_t c0_co, c1_co, c2_co, c0_inf, c1_inf, c2_inf, mu_eq, mu0_inf, mu1_inf,
      mu2_inf;
  ...
  real_t xmin_Container, xmax_Container, ymin_Container, ymax_Container,
      Width_Container;
  ...
  KOKKOS_INLINE_FUNCTION real_t phi2(real_t x) const {
    return 0.5 * (1 - tanh(sign * 2.0 * x / W));
  }
  ...
  KOKKOS_INLINE_FUNCTION real_t c0_02(real_t x) const {
    return 0.5 *
           (c2_inf + c0_inf + (c2_inf - c0_inf) * tanh(sign * 2.0 * x / W));
  }
  ...
  // KOKKOS_INLINE_FUNCTION real_t interp_rho(real_t phi) const {return
  // (1.0-phi) * rho0 + (phi * rho1);}
  ...
  KOKKOS_INLINE_FUNCTION real_t rho2c(real_t c) const {
    return (rho2_eq - rho2_ini) / (c2_co - c2_inf) * (c - c2_inf) + rho2_ini;
  }
  ...
  KOKKOS_INLINE_FUNCTION real_t rho2c_prime(real_t c) const {
    return (rho2_eq - rho2_ini) / (c2_co - c2_inf);
  }
  KOKKOS_INLINE_FUNCTION real_t interp_rho(real_t phi, real_t c) const {
    return (1.0 - hrho(phi)) * rho0c(c) + hrho(phi) * rho1c(c);
  }
  // KOKKOS_INLINE_FUNCTION real_t interp_rho02(real_t phi0,real_t phi2,real_t
  // c) const {return (phi0*rho0c(c) + phi2*rho2c(c));}
  ...
  KOKKOS_INLINE_FUNCTION real_t c_co_Exchange02(real_t phi0,
                                                real_t phi2) const {
    return c0_co * phi0 + c2_co * phi2;
  }
  KOKKOS_INLINE_FUNCTION real_t interpol_D02(real_t phi0, real_t phi2) const {
    return D0 * phi0 + D2 * phi2;
  }
  ...
  KOKKOS_INLINE_FUNCTION real_t Sigma12c(real_t c) const {
    return (Sigma12eq - Sigma12) / (c2_co - c2_inf) * (c - c2_inf) + Sigma12;
  }
  KOKKOS_INLINE_FUNCTION real_t Sigma13c(real_t c) const {
    return (Sigma13eq - Sigma13) / (c1_co - c1_inf) * (c - c1_inf) + Sigma13;
  }
  KOKKOS_INLINE_FUNCTION real_t Sigma23c(real_t c) const {
    return (Sigma23eq - Sigma23) / (c1_co - c1_inf) * (c - c1_inf) + Sigma23;
  }
  ...
  KOKKOS_INLINE_FUNCTION RVect<dim> S_ct(const LBMState &lbmState) const {
    const real_t norm = sqrt(SQR(lbmState[IDPHIDX]) + SQR(lbmState[IDPHIDY]) +
                        ...
                        NORMAL_EPSILON;
    real_t force_ct = Mphi * counter_term * 4.0 * lbmState[IPHI] *
  ...
  real_t S_st012(EquationTag1 tag, const LBMState &lbmState) const {
    real_t valmu =
        mu012(lbmState[IC], lbmState[IPHI3], lbmState[IPHI], lbmState[IPHI2]);
  ...
  template <int dim>
  KOKKOS_INLINE_FUNCTION RVect<dim> M1_PHI2(EquationTag4 tag,
                                            const LBMState &lbmState) const {
    RVect<dim> term;
    term[IX] = lbmState[IPHI2] * lbmState[IU];
    term[IY] = lbmState[IPHI2] * lbmState[IV];
    if (dim == 3) {
      term[IZ] = lbmState[IPHI2] * lbmState[IW];
    }
    return term;
  }
  ...
  real_t S2_dw(const LBMState &lbmState) const {
    real_t source_noct = -(1.0 - counter_term) * Mphi / SQR(W) * 16.0 *
                         lbmState[IPHI2] * (1.0 - lbmState[IPHI2]) *
                         (1.0 - 2.0 * lbmState[IPHI2]);
    return source_noct;
  }
  // Computation of macroscopic counter term with Phi2 (vector)
  template <int dim>
  KOKKOS_INLINE_FUNCTION RVect<dim> S2_ct(const LBMState &lbmState) const {
    const real_t norm = sqrt(SQR(lbmState[IDPHI2DX]) + SQR(lbmState[IDPHI2DY]) +
                             SQR(lbmState[IDPHI2DZ])) +
                        NORMAL_EPSILON;
    real_t force_ct = Mphi * counter_term * 4.0 * lbmState[IPHI2] *
                      (1.0 - lbmState[IPHI2]) / W / norm;
    RVect<dim> term;
  ...
  // Computation of macroscopic counter term with Phi3 (vector) for Eq phi and
  // Eq phi2
  template <int dim>
  KOKKOS_INLINE_FUNCTION RVect<dim> S3_ct(const LBMState &lbmState) const {
    const real_t norm = sqrt(SQR(lbmState[IDPHI3DX]) + SQR(lbmState[IDPHI3DY]) +
                             SQR(lbmState[IDPHI3DZ])) +
                        NORMAL_EPSILON;
    real_t force_ct = Mphi * counter_term * 4.0 * lbmState[IPHI3] *
                      (1.0 - lbmState[IPHI3]) / W / norm;
    RVect<dim> term;
  ...
  real_t tau_3phases_NS(const LBMState &lbmState) const {
    ...
    real_t tau =
        0.5 + (tau1 * tau2 * tau3) / (tau2 * tau3 * phi1 + tau1 * tau3 * phi2 +
                                      tau1 * tau2 * phi3);
  ...
  template <int dim>
  KOKKOS_INLINE_FUNCTION RVect<dim> force_P2(const LBMState &lbmState) const {
  ...
  template <int dim>
  KOKKOS_INLINE_FUNCTION RVect<dim>
  ...
  // =================== AJOUT ALAIN ====================================
  // Surface tension term number 1 for NS: three-phase case (implementation of
  // Eq. (16) in Ref)
  template <int dim>
  KOKKOS_INLINE_FUNCTION RVect<dim> force_TS1(const LBMState &lbmState) const {
    ...
    const real_t term2 =
        (1.0 / Gamma3) * ((12.0 * Gamma1 / W) * g2_prime(lbmState[IPHI]) -
                          (12.0 * Gamma3 / W) * g2_prime(lbmState[IPHI3]));
  ...
  KOKKOS_INLINE_FUNCTION RVect<dim> force_TS3(const LBMState &lbmState) const {
    ...
    const real_t factor = 4.0 * GammaT / W;
    const real_t term1 =
        (1.0 / Gamma1) * ((12.0 * Gamma3 / W) * g2_prime(lbmState[IPHI3]) -
                          (12.0 * Gamma1 / W) * g2_prime(lbmState[IPHI]));
    const real_t term2 =
        (1.0 / Gamma2) * ((12.0 * Gamma3 / W) * g2_prime(lbmState[IPHI3]) -
                          (12.0 * Gamma2 / W) * g2_prime(lbmState[IPHI2]));
    const real_t mu3_pot = factor * (term1 + term2) -
                           (3.0 / 4.0) * W * Gamma3 * lbmState[ILAPLAPHI3];
    RVect<dim> term;
  ...
  // =======================================================
  // Grad rho computed by the chain rule --> (grad phi) and (grad c) // Comments
  // ALAIN
  template <int dim>
  ...
  real_t tau_C02(TagCompo tag, const LBMState &lbmState) const {
    return 0.5 + (e2 * dt * interpol_D02(lbmState[IPHI3], lbmState[IPHI2]) /
                  (SQR(dx)));
  }
  // relaxation coef for LBM scheme of Navier-Stokes equation for three phases
  KOKKOS_INLINE_FUNCTION
