#ifndef MODELS_NS_SURFACTANT_H
#define MODELS_NS_SURFACTANT_H
#include "InitConditionsTypes.h"
#include "Index_NS_Surfactant.h"
#include <real_type.h>
...
struct ModelParams {
  ...
  ModelParams(const ConfigMap &configMap, LBMParams params) {
    ...
    //Read Parameters for surfactant
    beta_surf = configMap.getFloat("params_composition", "beta_surf", 0.0);
    k_surf = configMap.getFloat("params_composition", "k_surf", 0.0);
    eps_surf = configMap.getFloat("params_composition", "eps_surf", 0.0);
    Mcomp = configMap.getFloat("params_composition", "Mcomp", 0.0);
    M1orF = configMap.getInteger("params_composition", "M1orF", 0);//0 for the term to be in M1, 1 for it to be is the forcing term
    ...
    x1       = configMap.getFloat  ("init", "x1"         , 0.0);
    y1       = configMap.getFloat  ("init", "y1"         , 0.0);
    ...
    else if (initTypeStr == "surf_test1")
      initType = SURF_TEST1 ;
    else if (initTypeStr == "surf_test2")
      initType = SURF_TEST2 ;
  ...
  real_t beta_surf,k_surf,eps_surf,Mcomp;
  int M1orF;
  ...
  KOKKOS_INLINE_FUNCTION real_t c0(real_t x) const {
   ...
   KOKKOS_INLINE_FUNCTION real_t c_init(real_t x) const {
    return 0.5 *(c1_co + c0_co + (c1_co - c0_co) * tanh(sign * 2.0 * x / W));
  }
  ...
  real_t M2_MU_PHI(EquationTag1 tag, const LBMState &lbmState) const {
	real_t sigma12 = Marangoni==1?sigma_func(lbmState[IC]):sigma;
	const real_t mu_phi = sigma12 * 1.5 / W *
  ...
  KOKKOS_INLINE_FUNCTION RVect<dim> force_MA(const LBMState &lbmState) const {
	...
	if (dim == 3) {
		const real_t grad_phi_sqr = (SQR(lbmState[IDPHIDX])+SQR(lbmState[IDPHIDY]) +SQR(lbmState[IDPHIDZ])); //grad phi squared
		const real_t grad_sigmax = sigma_prime(lbmState[IC])*lbmState[IDCDX]; //d(sigma)/dx = d(sigma)/dC * d(C)/dx
		const real_t grad_sigmay = sigma_prime(lbmState[IC])*lbmState[IDCDY]; //d(sigma)/dy = d(sigma)/dC * d(C)/dy
		const real_t grad_sigmaz = sigma_prime(lbmState[IC])*lbmState[IDCDZ]; //d(sigma)/dy = d(sigma)/dC * d(C)/dz
    
		const real_t dot_prod = lbmState[IDPHIDX]*grad_sigmax + lbmState[IDPHIDY]*grad_sigmay  +  lbmState[IDPHIDZ]*grad_sigmaz;// grad sigma . grad phi 
		...
		term[IX] = 1.5*W*(grad_phi_sqr*grad_sigmax - dot_prod*lbmState[IDPHIDX]);
		term[IY] = 1.5*W*(grad_phi_sqr*grad_sigmay - dot_prod*lbmState[IDPHIDY]);
		term[IZ] = 1.5*W*(grad_phi_sqr*grad_sigmaz - dot_prod*lbmState[IDPHIDZ]);
		
		return term;
		}
	}
  //sigma as a function of the composition
  ...
  real_t M2_C(TagCompo tag, const LBMState &lbmState) const {
    return lbmState[IC];//gamma * interpol_D(lbmState[IPHI]) * lbmState[IMU];
  }
  ...
  real_t tau_C(TagSansGamma tag, const LBMState &lbmState) const {
    return 0.5 + (e2 * dt * Mcomp / (SQR(dx)));//0.5 + (e2 * dt * interpol_D(lbmState[IPHI]) / (SQR(dx)));
  }

  KOKKOS_INLINE_FUNCTION
  real_t M2_C(TagSansGamma tag, const LBMState &lbmState) const {
    return lbmState[IC];//lbmState[IMU];
  }
  ...
  //Flux for surfactant counter term in AC
  template <int dim>
  KOKKOS_INLINE_FUNCTION RVect<dim> CT_SURF_M1(const LBMState &lbmState) const {
    RVect<dim> term;
    const real_t norm = sqrt(SQR(lbmState[IDPHIDX]) + SQR(lbmState[IDPHIDY]) +
                             SQR(lbmState[IDPHIDZ])) +
                        NORMAL_EPSILON;
    real_t N  = (4*beta_surf)*lbmState[IPHI]*(1-lbmState[IPHI])*(0.5*k_surf*(1-2*lbmState[IPHI])+(16*eps_surf)*lbmState[IPHI]*(1-lbmState[IPHI])*(1-2*lbmState[IPHI])/SQR(W) - log(c0_co/c1_co)/(beta_surf+NORMAL_EPSILON) )/W;//-beta_surf*log(c1_co/c0_co))/W;
					//-beta_surf*log(c1_co/c0_co));
				
    real_t val_ct = (1-M1orF)*Mcomp*N*lbmState[IC]*(1-lbmState[IC])/norm ;
    
    term[IX] = val_ct * lbmState[IDPHIDX];
    term[IY] = val_ct * lbmState[IDPHIDY];
  
  
  

    if (dim == 3) {
      term[IZ] = val_ct * lbmState[IDPHIDZ];
    }
    return term;
  }
  //
  template <int dim>
  KOKKOS_INLINE_FUNCTION RVect<dim> S_ct_comp(const LBMState &lbmState) const {
    const real_t norm = sqrt(SQR(lbmState[IDPHIDX]) + SQR(lbmState[IDPHIDY]) +
                             SQR(lbmState[IDPHIDZ])) +
                        NORMAL_EPSILON;
    real_t N = (4*beta_surf)*lbmState[IPHI]*(1-lbmState[IPHI])*(0.5*k_surf*(1-2*lbmState[IPHI])+(16*eps_surf)*lbmState[IPHI]*(1-lbmState[IPHI])*(1-2*lbmState[IPHI])/SQR(W))/W;
    real_t force_ct = M1orF*Mcomp*N*lbmState[IC]*(1-lbmState[IC])/norm ;
    RVect<dim> term;
    term[IX] = force_ct * lbmState[IDPHIDX];
    term[IY] = force_ct * lbmState[IDPHIDY];
    if (dim == 3) {
  ...
  //parameters : beta_surf,k_surf,eps_surf
  //
  //============================//
  //============================//
