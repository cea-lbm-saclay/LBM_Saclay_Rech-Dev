...
#include "Models_NS_Surfactant.h"
...
//    }
  ...
  void setup_collider(EquationTag2 tag, const IVect<dim> &IJK,
    ...
    if (dim == 2) {
      ...
      ForceTot[IX] = ForceG[IX] + ForceP[IX] + ForceTS[IX] + ForceV[IX] + ForceMA[IX];
      ForceTot[IY] = ForceG[IY] + ForceP[IY] + ForceTS[IY] + ForceV[IY] + ForceMA[IY];
      ...
      this->set_lbm_val(IJK, IFTSX, ForceTS[IX]);
       this->set_lbm_val(IJK, IFTSY, ForceTS[IY]);
  ...
  void setup_collider(EquationTag2 tag, const IVect<dim> &IJK,
	...
	if (dim == 2) {
		...
		for (int ipop = 0; ipop < npop; ++ipop) {
			...
			Base::compute_scal(ipop, ForceTot[IX], ForceTot[IY]) - w[ipop]*dt*Sterm_mass_bal ;
  ...
  void setup_collider(EquationTag3 tag, const IVect<dim> &IJK,
    ...
    //
    const RVect<dim> M1_CT = Model.CT_SURF_M1<dim>(lbmState);
    //
    const real_t M2 = Model.M2_C(type, lbmState);
    const RVect<dim> S0 = Model.S_ct_comp<dim>(lbmState);//Model.S0_C<dim>(type, lbmState);
    ...
    // ipop = 0
    collider.f[0] = Base::get_f_val(tag, IJK, 0);
    collider.S0[0] = w[0] * dt*staudx * Base::compute_scal(0, S0);
    collider.feq[0] = M0 - (1 - w[0]) * M2 - 0.5 * collider.S0[0];

    // ipop > 0
    for (int ipop = 1; ipop < npop; ++ipop) {
      collider.f[ipop] = this->get_f_val(tag, IJK, ipop);
      collider.S0[ipop] = w[ipop] * dt*staudx* Base::compute_scal(ipop, S0);
      collider.feq[ipop] =
          w[ipop] * (M2 + c_cs2 * Base::compute_scal(ipop, M1)+ c_cs2 * Base::compute_scal(ipop, M1_CT)) -
          0.5 * collider.S0[ipop];
    }
  }
  ...
  void init_macro(IVect<dim> IJK, RANDOM_POOL::generator_type rand_gen) const {
    ...
    if (Model.initType == PHASE_FIELD_INIT_VERTICAL) {
    ...
    else if (Model.initType == SURF_TEST1) {
	  if (dim == 2) {
		xphi = Model.L/2 - x;
		xc = xphi;
		c = Model.c_init(xc);
     ...
     else if (Model.initType == SURF_TEST2) {
		 if (dim == 2){
				real_t d1,d2;
				d1 = (Model.r0 - sqrt(SQR(x - Model.x0) + SQR(y - Model.y0)));
				d2 = (Model.r1 - sqrt(SQR(x - Model.x1) + SQR(y - Model.y1)));
				xphi = FMAX(d1,d2);
				c = Model.c0_co*(1 + 0.0*(rand_gen.drand()-0.5));
			}
    }
...
//
    ...
    if((Model.initType != SURF_TEST1)&&(Model.initType != SURF_TEST2)&&(Model.initType != RISING_BUBBLE)&&(Model.initType != PHASE_FIELD_INIT_TAYLOR)){
		c  = Model.c0(xc);
		}
    ...
    this->set_lbm_val(IJK, ISIGMA, Model.sigma_func(c));
	// ****************************************************************************************************
	//
	...
	if (Model.initType == TWO_PLANE) {
      ...
      if (dim == 2) {
        real_t h=Model.y_dom_max - Model.y_dom_min;
        vx = Model.U0*((y-h/2.0)/(h/2.0));
      }
    }
