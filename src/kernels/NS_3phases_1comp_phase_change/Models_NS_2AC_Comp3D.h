#ifndef MODELS_NS_2AC_C_3D_H_
#define MODELS_NS_2AC_C_3D_H_
#include "InitConditionsTypes.h"
#include "Index_NS_2AC_Comp3D.h"
#include <real_type.h>
...
struct ModelParams {
    ...
    void showParams()
    {
        if (myRank == 0) {
            ...
            std::cout << "tauC0 :    " << (0.5 + (e2 * dt * (D0) / (SQR(dx)))) << std::endl;
            std::cout << "tauC1 :    " << (0.5 + (e2 * dt * (D1) / (SQR(dx)))) << std::endl;
            std::cout << "tauC2 :    " << (0.5 + (e2 * dt * (D2) / (SQR(dx)))) << std::endl;
            std::cout << "cs :    " << ((dx / dt) / SQRT(e2)) << std::endl;
            std::cout << "cs2 :    " << SQR(dx / dt) / e2 << std::endl;
            std::cout << "counter_term :    " << counter_term << std::endl;
            std::cout << "lambda01 :    " << lambda01 << std::endl;
            std::cout << "lambda12 :    " << lambda12 << std::endl;
            std::cout << "lambda02 :    " << lambda02 << std::endl;
    ...
    ModelParams(const ConfigMap& configMap, LBMParams params)
    {
        ...
        lambda01 = configMap.getFloat("params", "lambda01", 0.0);
        lambda12 = configMap.getFloat("params", "lambda12", 0.0);
        lambda02 = configMap.getFloat("params", "lambda02", 0.0);
        Mphi = configMap.getFloat("params", "Mphi", 1.2);
        
        m0 = configMap.getFloat("params_thermo", "m0", 1);
        q0 = configMap.getFloat("params_thermo", "q0", 0);
        m1 = configMap.getFloat("params_thermo", "m1", 1);
        q1 = configMap.getFloat("params_thermo", "q1", 0);
        m2 = configMap.getFloat("params_thermo", "m2", 1);
        q2 = configMap.getFloat("params_thermo", "q2", 0);
        ...
        c1_01 = configMap.getFloat("params_composition", "c1_01", 0.0);
        c1_12 = configMap.getFloat("params_composition", "c1_12", 0.0);
        ...
        C_advec = configMap.getFloat("params", "C_advec", 0.0);
		...
		x01 = configMap.getFloat("init", "x01", 0.0);
		x12 = configMap.getFloat("init", "x12", 0.0);
		init_time = configMap.getFloat("init", "init_time", 0.0);
		xi01 = configMap.getFloat("init", "xi01", 0.0);
		xi12 = configMap.getFloat("init", "xi12", 0.0);
		mu01 = configMap.getFloat("init", "mu01", 0.0);
		mu12 = configMap.getFloat("init", "mu12", 0.0);

		init_sharp = configMap.getBool("init", "init_sharp", 0.0);
        ...
        else if (initTypeStr == "vertical_3_phases")
            initType = PHASE_FIELD_INIT_VERTICAL_3_PHASES;
        else if (initTypeStr == "vertical_3_phases_erfc_init")
            initType = PHASE_FIELD_INIT_VERTICAL_3_PHASES_ERFC;
        ...
        else if (initTypeStr == "rayleigh_taylor_3D")
            initType = PHASE_FIELD_INIT_TAYLOR_3D;
        ...
        else if (initTypeStr == "rising_droplets_3D")
            initType = THREEPHASES_RISING_DROPLETS_3D;
    ...
    real_t W, Mphi; // Parameters of phase-field equation
    real_t lambda01, lambda12, lambda02; // Parameters of phase-field equation
    real_t m0, m1, m2, q0, q1, q2; // Parameters of quadratic thermodynamical models
    ...
    real_t x01,x12;
    bool init_sharp;
    real_t init_time, xi01, xi12, mu01, mu12;  // for analytical solution init
    real_t ampl, height, heightPhi2, width, n_wave, wave_length;
    real_t initVX_upper, initVX_lower, U0, L0;
    real_t xC, yC, zC, R, xC1, yC1, zC1, R1, xC2, yC2, zC2, R2;
    ...
    KOKKOS_INLINE_FUNCTION real_t p_prime(real_t phi) const { return 6.0 * phi * (1.0 - phi)*(phi>0)*(phi<1); }
    ...
    real_t w0(real_t mu) const
    {
        return  -0.5*mu*mu-mu*m0+q0;
    }
    KOKKOS_INLINE_FUNCTION
    real_t w1(real_t mu) const
    {
        return  -0.5*mu*mu-mu*m1+q1;
    }
    KOKKOS_INLINE_FUNCTION
    real_t w2(real_t mu) const
    {
        return  -0.5*mu*mu-mu*m2+q2;
    }
    KOKKOS_INLINE_FUNCTION
    real_t fun_mu(real_t c, real_t phi1, real_t phi2) const
    {
        return  c - (m0 * (1-phi1-phi2) + m1 * phi1 + m2 * phi2);
    }
    // ================================================
    //
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        M1_PHI(EquationTag1 tag, const LBMState& lbmState) const
    {
        RVect<dim> term;
        term[IX] = lbmState[IPHI] * 0;
        term[IY] = lbmState[IPHI] * 0;
        if (dim == 3) {
            term[IZ] = lbmState[IPHI] * 0;
        }
        return term;
    }
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        S_ct(const LBMState& lbmState) const
    {
    ...
    KOKKOS_INLINE_FUNCTION
    real_t S_phase_change_phi1(const LBMState& lbmState) const
    {
		real_t phi1=lbmState[IPHI];
		real_t phi2=lbmState[IPHI2];
		real_t phi0=lbmState[IPHI3];
		real_t c=lbmState[IC];
		real_t valmu=fun_mu(c,phi1,phi2);
        //~ real_t S = ( p_prime(phi1) * w1(valmu) - p_prime(phi2) * w2(valmu) - p_prime(phi0) * w0(valmu) );
        real_t S = ( p_prime(phi1) * (lambda12 * p_prime(phi2) *(w1(valmu)-w2(valmu)) + lambda01 * p_prime(phi0) *(w1(valmu)-w0(valmu)) ));
        return -Mphi / SQR(W) * S;
    }
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        M1_PHI2(EquationTag4 tag, const LBMState& lbmState) const
    {
        RVect<dim> term;
        term[IX] = lbmState[IPHI2] * 0;
        term[IY] = lbmState[IPHI2] * 0;
        if (dim == 3) {
            term[IZ] = lbmState[IPHI2] * 0;
        }
        return term;
    }
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        S2_ct(const LBMState& lbmState) const
    {
        const real_t norm = sqrt(SQR(lbmState[IDPHI2DX]) + SQR(lbmState[IDPHI2DY]) + SQR(lbmState[IDPHI2DZ])) + NORMAL_EPSILON;
        ...
        if (dim == 3) {
            term[IZ] = force_ct * lbmState[IDPHI2DZ];
        }
        return term;
    }
    // Coupling with thermodynamic (equilibirum compositions)
    KOKKOS_INLINE_FUNCTION
    real_t S_phase_change_phi2(const LBMState& lbmState) const
    {
		real_t phi1=lbmState[IPHI];
		real_t phi2=lbmState[IPHI2];
		real_t phi0=lbmState[IPHI3];
		real_t c=lbmState[IC];
		real_t valmu=fun_mu(c,phi1,phi2);
        //~ real_t S = ( p_prime(phi2) * w2(valmu) - p_prime(phi1) * w1(valmu) - p_prime(phi0) * w0(valmu) );
        real_t S = ( p_prime(phi2) * (lambda12 * p_prime(phi1) *(w2(valmu)-w1(valmu)) + lambda02 * p_prime(phi0) *(w2(valmu)-w0(valmu)) ));
        return - Mphi / SQR(W) * S;
    }
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        S3_ct(const LBMState& lbmState) const
    {
        const real_t norm = sqrt(SQR(lbmState[IDPHI3DX]) + SQR(lbmState[IDPHI3DY]) + SQR(lbmState[IDPHI3DZ])) + NORMAL_EPSILON;
        ...
        if (dim == 3) {
            term[IZ] = force_ct * lbmState[IDPHI3DZ];
        }
        return term;
    }
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        force_P2(const LBMState& lbmState) const
    {
        const RVect<dim> gradrho = grad_rho<dim>(lbmState);
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        force_P_rhonum(const LBMState& lbmState) const
    {
        //const RVect<dim> gradrho = grad_rho(lbmState);
        RVect<dim> gradrho;
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        force_G(const LBMState& lbmState) const
    {
        RVect<dim> term;
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        force_TS(const LBMState& lbmState) const
    {
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        force_TS1(const LBMState& lbmState) const
    {
        real_t c = lbmState[IC];
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        force_TS2(const LBMState& lbmState) const
    {
        real_t c = lbmState[IC];
        real_t Gamma1 = Sigma12c(c) + Sigma13 - Sigma23;
        real_t Gamma2 = Sigma12c(c) + Sigma23 - Sigma13;
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        force_TS3(const LBMState& lbmState) const
    {
        real_t c = lbmState[IC];
        real_t Gamma1 = Sigma12c(c) + Sigma13 - Sigma23;
        real_t Gamma2 = Sigma12c(c) + Sigma23 - Sigma13;
        ...
        if (dim == 3) {
            term[IZ] = mu3_pot * lbmState[IDPHI3DZ];
        }
        return term;
    }
    // =================== AJOUT ALAIN ====================================
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        force_Beckermann(const LBMState& lbmState) const
    {
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        grad_rho(const LBMState& lbmState) const
    {
        RVect<dim> term;
    ...
    //	AJOUT ALAIN
    KOKKOS_INLINE_FUNCTION
    real_t tau_C02(TagCompo tag, const LBMState& lbmState) const
    {
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        M1_C(const LBMState& lbmState) const
    {
        RVect<dim> term;
        term[IX] = lbmState[IC] * 0;
        term[IY] = lbmState[IC] * 0;
        if (dim == 3) {
            term[IZ] = lbmState[IC] * 0;
        }
        return term;
    }
    ...
    {
        return gamma * lbmState[IMU];
    }

    template <int dim>
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        S0_C(TagCompo tag, const LBMState& lbmState) const
    {
        RVect<dim> term;
        real_t coeff = 0*(D1 - D0) * q_prime(lbmState[IPHI]) * lbmState[IMU];
    ...
    real_t tau_C(TagSansGamma tag, const LBMState& lbmState) const
    {
        ...
        const real_t phi2 = lbmState[IPHI2];
        const real_t phi3 = lbmState[IPHI3];
        return 0.5 + (e2 * dt * (D0*phi3 + D1*phi1+D2*phi2) / (SQR(dx)));
    }
    //	AJOUT ALAIN
    KOKKOS_INLINE_FUNCTION
    real_t tau_C02(TagSansGamma tag, const LBMState& lbmState) const
    {
		const real_t phi1 = lbmState[IPHI];
        const real_t phi2 = lbmState[IPHI2];
        const real_t phi3 = lbmState[IPHI3];
    ...
    KOKKOS_INLINE_FUNCTION
        RVect<dim>
        S0_C(TagSansGamma tag, const LBMState& lbmState) const
    {
        RVect<dim> term;
        term[IX] = 0.0;
        term[IY] = 0.0;
