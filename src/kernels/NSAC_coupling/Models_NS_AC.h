/**
 * \file Models_NS_AC.h
 * \brief Model class with all parameters and physical computations
 *
 */

#ifndef MODELS_NS_PF_H_
#define MODELS_NS_PF_H_
#include "InitConditionsTypes.h"
#include "Index_NS_AC.h"
#include <real_type.h>
namespace PBM_NSAC {
// ================================================
//
...
struct ModelParams {
    ...
    ModelParams(const ConfigMap& configMap, LBMParams params)
    {
        ...
        x1 = configMap.getFloat("init", "x1", 0);
        y1 = configMap.getFloat("init", "y1", 0);
        z1 = configMap.getFloat("init", "z1", 0);
        ...
        CC_phi_threshold = configMap.getFloat("params", "CC_phi_threshold", 0.5);
        ...
        else if (initTypeStr == "ring")
            initType = PHASE_FIELD_INIT_RING;
    ...
    real_t dx, dt, e2, cs2, time;
    real_t nu1, rho1, nu0, rho0, shape_coef;
    ...
    KOKKOS_INLINE_FUNCTION
    RVect2 force_NS(Tag2Quadra tag, const LBMState& lbmState) const
    {
    ...
    KOKKOS_INLINE_FUNCTION
    RVect2 force_P(Tag2Quadra tag, const LBMState& lbmState) const
    {
        const real_t cs2 = SQR(dx / dt) / e2;
        //~ const real_t rho =lbmState[ID];
        //~ const real_t drhocs2 = cs2*(rho1 - rho0)*SQR(rho)/rho1/rho0;
        const real_t drhocs2 = cs2 * (rho1 - rho0);
        RVect2 term;
    ...
    KOKKOS_INLINE_FUNCTION
    real_t tau_NS(Tag2Quadra tag, const LBMState& lbmState) const
    {
        const real_t phi = lbmState[IPHI];
