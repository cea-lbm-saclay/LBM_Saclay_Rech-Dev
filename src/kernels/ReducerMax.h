#ifndef MAX_FUNCTOR_H_
#define MAX_FUNCTOR_H_
...
public:
    ...
    MaxReducer(const LBMSchemeSpec& scheme, int isize, int jsize, int ksize, int ghostWidth, int ifield1, int ifield2)
        : isize(isize)
        , jsize(jsize)
        , ksize(ksize)
        , gw(ghostWidth)
    ...
    inline static void apply(const LBMParams& params, const LBMSchemeSpec& scheme, int ifield1, int ifield2, real_t& maxR)
    {

        const int nbCells = dim == 2 ? params.isize * params.jsize : params.isize * params.jsize * params.ksize;

        MaxReducer functor(scheme, params.isize, params.jsize, params.ksize, params.ghostWidth, ifield1, ifield2);

        real_t max;

        Kokkos::RangePolicy<int> policy(0, nbCells);
        Kokkos::parallel_reduce("MaxReduce", policy, functor, Kokkos::Max<real_t>(max));
    ...
    KOKKOS_INLINE_FUNCTION void operator()(const typename std::enable_if<dim_ == 2, int>::type& index, typename std::enable_if<dim_ == 2, real_t>::type& lmax) const
    {
        IVect<dim> IJK;
        index2coord(index, IJK, isize, jsize);

        real_t valX = scheme.get_lbm_val(IJK, ifield1);
        real_t valY = scheme.get_lbm_val(IJK, ifield2);
        real_t norm = sqrt(valX * valX + valY * valY);

        if ((norm > lmax) && is_in_bounds(IJK))
            lmax = norm;
    }
    ...
    template <int dim_ = dim>
    KOKKOS_INLINE_FUNCTION void operator()(const typename std::enable_if<dim_ == 3, int>::type& index, typename std::enable_if<dim_ == 3, real_t>::type& lmax) const
    {
        IVect<dim> IJK;
