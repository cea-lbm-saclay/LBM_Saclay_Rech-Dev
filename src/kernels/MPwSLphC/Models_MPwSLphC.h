...
struct ModelParams {
  ...
  ModelParams(const ConfigMap &configMap, LBMParams params) {
	...
	x0_liquid_rect     = configMap.getFloat("init","x0_liquid_rect"   , 0.0);
	y0_liquid_rect     = configMap.getFloat("init","y0_liquid_rect"   , 0.0);
	z0_liquid_rect     = configMap.getFloat("init","z0_liquid_rect"   , 0.0);
	Larg_liquid_rect   = configMap.getFloat("init","Larg_liquid_rect" , 0.0);
	long_liquid_rect   = configMap.getFloat("init","long_liquid_rect" , 0.0);
	haut_liquid_rect   = configMap.getFloat("init","haut_liquid_rect" , 0.0);
	theta_liquid_rect  = configMap.getFloat("init","theta_liquid_rect", 0.0);
    ...
    solid_phase         = configMap.getInteger("param_solid","solid_phase"        , 0  );
    diffuse_bounce_back = configMap.getInteger("param_solid","diffuse_bounce_back", 0  );
    W_sol               = configMap.getFloat  ("param_solid","W_sol"              , 1.0);
    Mphi_sol            = configMap.getFloat  ("param_solid","Mphi_sol"           , 1.0);
	rho_sol             = configMap.getFloat  ("param_solid","rho_sol"            , 1.0);
	nu_sol              = configMap.getFloat  ("param_solid","nu_sol"             , 0.0);
	c_sol_co            = configMap.getFloat  ("param_solid","c_sol_co"           , 0.0);
	D_sol               = configMap.getFloat  ("param_solid","D_sol"              , 1.0);
	sign_sol            = configMap.getFloat  ("param_solid","sign_sol"           , 1.0);
	...
	KR_W0                    = configMap.getFloat  ("crystal_growth","KR_W0"          ,1.0);
	KR_tau0                  = configMap.getFloat  ("crystal_growth","KR_tau0"        ,1.0);
	KR_lambda0               = configMap.getFloat  ("crystal_growth","KR_lambda0"     ,1.0);
	KR_eps                   = configMap.getFloat  ("crystal_growth","KR_eps"         ,1.0);
	KR_diffusivity           = configMap.getFloat  ("crystal_growth","KR_diffusivity" ,1.0);
	KR_counter_term          = configMap.getInteger("crystal_growth","KR_counter_term",0  );
	coeff_source_term_compos = configMap.getFloat  ("crystal_growth","st_compos"      ,0.0);
    ...
    else if (initTypeStr == "rectangle")
      initType = PHASE_FIELD_INIT_RECTANGLE;
  ...
  real_t xC, yC, zC, R;
  real_t x0_liquid_rect, y0_liquid_rect, z0_liquid_rect, Larg_liquid_rect, long_liquid_rect, haut_liquid_rect, theta_liquid_rect;
  //! ===================================== init solid phase =========================================
  int solid_phase, move_solid, initTypeSol, initMoveSolid;
  int diffuse_bounce_back, lagrange_multiplier, contact_angle, injector;
  real_t Mphi_sol, W_sol, rho_sol, nu_sol, D_sol, c_sol_co,sign_sol, coeff_solid_penalization;
  ...
  real_t KR_W0, KR_tau0, KR_lambda0, KR_eps, KR_diffusivity, KR_counter_term, coeff_source_term_compos ;
  //! MRT params
  ...
  KOKKOS_INLINE_FUNCTION real_t c_co(real_t phi, real_t phi2, real_t psi) const {
    return c1_co*phi + c0_co*phi2 + c_sol_co*psi;
  }
  ...
  KOKKOS_INLINE_FUNCTION RVect<dim> S_ct_psi(const LBMState &lbmState) const {
    const real_t norm = sqrt(SQR(lbmState[IDPSIDX]) + SQR(lbmState[IDPSIDY]) +
                             SQR(lbmState[IDPSIDZ])) +
                        NORMAL_EPSILON;
    real_t factr  = Mphi_sol*lbmState[IPSI] + Mphi*(1.-lbmState[IPSI]) ;
    real_t val_ct = factr * KR_counter_term * 4.0 * lbmState[IPSI] *
                      (1.0 - lbmState[IPSI]) / (W * norm);
    RVect<dim> term;
    term[IX] = val_ct * lbmState[IDPSIDX];
    term[IY] = val_ct * lbmState[IDPSIDY];
    if (dim == 3) {
      term[IZ] = val_ct * lbmState[IDPSIDZ];
    }
    return term;
  }
  // =======================================================
  // Source term for Conserv Allen-Cahn coupled with GP diffusion
  KOKKOS_INLINE_FUNCTION
  real_t S_st(EquationTag1 tag, const LBMState &lbmState) const {
	real_t phi  = lbmState[IPHI];
	real_t phi2 = lbmState[IPHI2];
	real_t psi  = lbmState[IPSI];
	real_t c    = lbmState[IC];
    real_t S = p_prime(lbmState[IPHI]) * (c0_co - c1_co) *
               (mu(c, phi, phi2, psi) - mu_eq);
    return -lambda * Mphi / SQR(W) * S;
  }
  // =======================================================
  // Source term for CAC eq coupled with PF PSI and Tempr
  KOKKOS_INLINE_FUNCTION
  real_t St_phi(EquationTag1 tag, const LBMState &lbmState) const 
  {
    //real_t S = p_prime(lbmState[IPSI]) * p_prime(lbmState[IPHI]) * lbmState[IC] ;
    real_t S = p_prime(lbmState[IPSI]) * lbmState[IC] ;
    return lambda * Mphi / SQR(W) * S;
  }
  // =======================================================
  ...
  KOKKOS_INLINE_FUNCTION RVect<dim> Lagrange_mult (const LBMState &lbmState) const {
    ...
    real_t factr   = (solid_phase / 3.0) * (Mphi_sol*lbmState[IPSI] + Mphi*(1.-lbmState[IPSI])) ;
    ...
    //lagr_mult[IX] = factr * (CT_phi1*lbmState[IDPHIDX] + CT_phi2*lbmState[IDPHI2DX]);
	//lagr_mult[IY] = factr * (CT_phi1*lbmState[IDPHIDY] + CT_phi2*lbmState[IDPHI2DY]);
  ...
  // Initialize phi as a rectangle 2D
  KOKKOS_INLINE_FUNCTION
  real_t phi_init_Rectangle2D(real_t x, real_t y) const {
	real_t xphi = 0.0 ;
	real_t x0    = x0_liquid_rect;
	real_t y0    = y0_liquid_rect;
	real_t L     = Larg_liquid_rect;
	real_t l     = long_liquid_rect;
	real_t theta = theta_liquid_rect ;
    ...
    if ( sqrt(SQR(xr-x0)) <= l/2 or sqrt(SQR(yr-y0)) <= L/2 ){
         xphi = FMIN(l/2 - sqrt(SQR(x0-xr)),L/2 - sqrt(SQR(y0-yr)));
    } else{
		if (yr > y0 + L/2){
			if(xr < x0 -l/2){
				xphi = -sqrt(SQR(xr - (x0-l/2)) + SQR(yr - (y0+L/2)));
			}
            if(xr > x0 + l/2){
                xphi = -sqrt(SQR(xr - (x0+l/2)) + SQR(yr - (y0+L/2)));
            }
  ...
  // relaxation coef for LBM scheme of phase field equation PSI
  KOKKOS_INLINE_FUNCTION
  real_t tau_PSI(EquationTag4 tag, const LBMState& lbmState, real_t As2) const
	{
		real_t tau = 0.5 + (3.0 * As2 * ((KR_W0*KR_W0)/KR_tau0) * dt / (dx * dx));
		return (tau);
	}
  // =======================================================
  // zero order moment for phase field PSI
  KOKKOS_INLINE_FUNCTION
  real_t M0_PSI(EquationTag4 tag, const LBMState& lbmState) const
	{
		return lbmState[IPSI];
	}
	
  // =======================================================
  // second order moment for phase field PSI
  KOKKOS_INLINE_FUNCTION
  real_t M2_PSI(EquationTag4 tag, const LBMState& lbmState) const
	{
		return lbmState[IPSI];
	}

  // =======================================================
  // Source term for phase field 0 < PSI < +1: S=lambda*p'(psi)*p'(phi)*T
  KOKKOS_INLINE_FUNCTION
  real_t St_psi (EquationTag4 tag, const LBMState& lbmState) const
  {	
	//real_t S = p_prime(lbmState[IPSI]) * p_prime(lbmState[IPHI]) * lbmState[IC];
	real_t S = p_prime(lbmState[IPSI]) * lbmState[IC];
    return - lambda * Mphi / SQR(W) * S;
  ...
  // Derivative of double-well
  KOKKOS_INLINE_FUNCTION
  real_t Sdw_psi (EquationTag4 tag, const LBMState &lbmState) const {
    real_t deriv_dw = - 16.0 * lbmState[IPSI] * (1.0 - lbmState[IPSI]) *
                         (1.0 - 2.0 * lbmState[IPSI]) / KR_tau0 ;
    return deriv_dw;
  }
  // =======================================================
  // Anisotropy function a_s(n)
  template<int dim>
  KOKKOS_INLINE_FUNCTION
  real_t compute_anisotropy(EquationTag4 tag, const LBMState& lbmState) const
	{
		const real_t norm = sqrt( SQR(lbmState[IDPSIDX]) + SQR(lbmState[IDPSIDY]))+NORMAL_EPSILON;
		real_t As_n;
		real_t xx4 = SQR(lbmState[IDPSIDX])*SQR(lbmState[IDPSIDX]);
		real_t yy4 = SQR(lbmState[IDPSIDY])*SQR(lbmState[IDPSIDY]);
		real_t zz4 = SQR(lbmState[IDPSIDZ])*SQR(lbmState[IDPSIDZ]);
		real_t ww4 = SQR(norm)*SQR(norm);
		if(dim==2){
			As_n = 1.0 - 3.0*KR_eps + 4.0*KR_eps*(xx4+yy4)/ww4;
		}
		else if(dim==3){
			As_n = 1.0 - 3.0*KR_eps + 4.0*KR_eps*(xx4+yy4+zz4)/ww4;
		}
		return As_n;
	}

  // =======================================================
  // Anisotropy vector phase-field eq PSI

  template<int dim>
  KOKKOS_INLINE_FUNCTION
  RVect<dim> compute_anisotropy_vector(EquationTag4 tag, const LBMState& lbmState) const
	{
		real_t xx = lbmState[IDPSIDX];
		real_t yy = lbmState[IDPSIDY];
		...
		if(dim==2){
			...
			real_t zz = lbmState[IDPSIDZ];
			real_t zz4 = zz*zz*zz*zz;
			const real_t norm = sqrt( SQR(lbmState[IDPSIDX]) + SQR(lbmState[IDPSIDY])+ SQR(lbmState[IDPSIDZ]))+NORMAL_EPSILON;
			real_t ww4 = SQR(norm)*SQR(norm);
			real_t ww6 = ww4*SQR(norm);
			real_t As_n = 1.0 - 3.0*KR_eps + 4.0*KR_eps*(xx4+yy4+zz4)/ww4;
			N[IX] = - 16.0*KR_eps*xx*(yy4-SQR(xx)*SQR(yy)-SQR(xx)*SQR(zz)+zz4)/ww6;
			N[IY] = - 16.0*KR_eps*yy*(xx4-SQR(yy)*SQR(xx)-SQR(yy)*SQR(zz)+zz4)/ww6;
			N[IZ] = -16.0*KR_eps*zz*(xx4-SQR(zz)*SQR(xx)-SQR(zz)*SQR(yy)+yy4)/ww6;
			N[IX] *= norm*norm*As_n;
			N[IY] *= norm*norm*As_n;
			N[IZ] *= norm*norm*As_n;
		}
  ...
  KOKKOS_INLINE_FUNCTION
  real_t mu(real_t c, real_t phi, real_t phi2, real_t psi) const { return mu_eq + c - c_co(phi,phi2,psi); }
  ...
  KOKKOS_INLINE_FUNCTION
  real_t M2C_Mu(TagCompo tag, const LBMState &lbmState) const {
    return gamma * interpol_D(lbmState[IPHI]) * lbmState[IMU];
  }
  
  KOKKOS_INLINE_FUNCTION
  real_t M2C_C(const LBMState &lbmState) const {
    return lbmState[IC];
  }

  // Source term for temperature (or composition) equation) of crystal growth
  KOKKOS_INLINE_FUNCTION
  real_t St_C (const LBMState &lbmState) const {
    real_t S = coeff_source_term_compos * lbmState[IDPSIDT] ;
    return S;
  }
  ...
  real_t tau_C(TagSansGamma tag, const LBMState &lbmState) const {
	...
	real_t phi2 = lbmState[IPHI2];
	real_t psi  = lbmState[IPSI];
	real_t D    = D1*phi + D0*phi2 + D_sol*psi ;  
    return 0.5 + (e2 * dt * D / (SQR(dx)));
  }

  KOKKOS_INLINE_FUNCTION
  real_t M2C_Mu_SansGamma(TagSansGamma tag, const LBMState &lbmState) const {
    return lbmState[IMU];
  }
  ...
  real_t S_stt(const LBMState &lbmState) const {
    real_t S = p_prime(lbmState[IPHI]) * (c0_co - c1_co) *
               (mu(lbmState[IC],lbmState[IPHI],lbmState[IPHI2],lbmState[IPSI]) - mu_eq);
    return -lambda * Mphi / SQR(W) * S;
  }
