...
struct Problem : public ProblemBase<dim, npop> {
  ...
  Problem(ConfigMap &configMap, LBMParams &params)
      : ProblemBase<dim, npop>(configMap, params, index2names::get_id2names()),
        params(params), configMap(configMap) {
  ...
  template <typename EquationTag, typename Collider> void update1eq() {
    using Kernel =
        SchemeFunctor<dim, npop, LBMSchemeSpec, EquationTag, Collider>;
    Kernel::template apply<TagUpdate>(params, scheme);
    EquationTag tag;
    scheme.swap_distribution(tag);
  }

  template <typename EquationTag, typename Collider> void init1eq() {
    using Kernel =
        SchemeFunctor<dim, npop, LBMSchemeSpec, EquationTag, Collider>;
  ...
  void init_f() override {
    ...
    if (params.collisionType4 == BGK) {
      init1eq<EquationTag4, BGKColliderTimeFactor<dim, npop>>();
    }
  ...
  void update_f() override {
    ...
    // init distribution functions psi
    if (params.collisionType4 == BGK) {
      update1eq<EquationTag4, BGKColliderTimeFactor<dim, npop>>();
    }
  ...
  void update_dt() override {
    ...
    real_t possible_dt =
        scheme.Model.fMach * scheme.Model.dx / sqrt(3) /
        sqrt(umax * umax / scheme.Model.L - scheme.Model.gy / scheme.Model.L) /
        scheme.Model.W;
