...
struct LBMScheme : public LBMSchemeBase<dim, npop> {
    ...
    void setup_collider(EquationTag1 tag, const IVect<dim>& IJK, BGK_Collider& collider) const
    {
        ...
        // AJOUTS ALAIN
        RVect2 ForceTS1 = Model.force_TS1(lbmState);
        RVect2 ForceTS2 = Model.force_TS2(lbmState); // AJOUTS ALAIN
        RVect2 ForceTS3 = Model.force_TS3(lbmState); // AJOUTS ALAIN
        // FIN AJOUTS ALAIN
        RVect2 ForceG = Model.force_G(lbmState);
        ...
        for (int alpha = 0; alpha < npop; ++alpha) {
            ForceV[IX] += E[alpha][IX] * E[alpha][IX] * lbmState[IDRHODX] * (collider.f[alpha] - collider.feq[alpha]) + E[alpha][IX] * E[alpha][IY] * lbmState[IDRHODY] * (collider.f[alpha] - collider.feq[alpha]);
            ForceV[IY] += E[alpha][IY] * E[alpha][IX] * lbmState[IDRHODX] * (collider.f[alpha] - collider.feq[alpha]) + E[alpha][IY] * E[alpha][IY] * lbmState[IDRHODY] * (collider.f[alpha] - collider.feq[alpha]);
        }
        ...
        for (int ipop = 0; ipop < npop; ++ipop) {
            collider.S0[ipop] = dt * w[ipop] / (lbmState[ID] * cs2) * Base::compute_scal(ipop, ForceTot[IX], ForceTot[IY]);
            ;
    ...
    void setup_collider(EquationTag1 tag, const IVect<dim>& IJK, MRT_Collider& collider) const
    {
        ...
        //RVect2 ForceTS = Model.force_TS(lbmState);
        RVect2 ForceTS1 = Model.force_TS1(lbmState); // AJOUTS ALAIN
        RVect2 ForceTS2 = Model.force_TS2(lbmState); // AJOUTS ALAIN
        RVect2 ForceTS3 = Model.force_TS3(lbmState); // AJOUTS ALAIN
        [[maybe_unused]] RVect2 ForceBck = Model.force_Beckermann(lbmState);
        //RVect2 ForceP = Model.force_P2(lbmState);
        RVect2 ForceP = Model.force_P_rhonum(lbmState); // MODIF ALAIN
        // FIN MODIFS ALAIN
        ...
        for (int beta = 0; beta < npop; ++beta) {
            ForceV[IX] += E[beta][IX] * E[beta][IX] * gradrho[IX] * MinvSMg_geq[beta] + E[beta][IX] * E[beta][IY] * gradrho[IY] * MinvSMg_geq[beta];
            ForceV[IY] += E[beta][IY] * E[beta][IX] * gradrho[IX] * MinvSMg_geq[beta] + E[beta][IY] * E[beta][IY] * gradrho[IY] * MinvSMg_geq[beta];
        }
    ...
    void make_boundary(const IVect<dim>& IJK, int faceId) const
    {
        ...
        } else if (Base::params.boundary_types[BOUNDARY_EQUATION_1][faceId] == BC_POISEUILLE) {
            const real_t dx = this->params.dx;
            const real_t dt = this->params.dt;
