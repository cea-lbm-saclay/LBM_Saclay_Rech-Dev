#ifndef LBM_SACLAY_IO_HDF5_H_
#define LBM_SACLAY_IO_HDF5_H_
 ...
 *
 * \param[in] params a LBMParams struct (to retrieve geometry).
 * \param[in] configMap
 * \param[in] index2names unstructured map of pairs (index, macroscopic variable name)
 ...
 *  the last step.
 *
 * index2names provides the list of macroscopic variables saved in hdf5 file.
 *
...
public:
    ...
    using LBMArray = typename std::conditional<d == TWO_D, LBMArray2d, LBMArray3d>::type;
    using LBMArrayHost = typename std::conditional<d == TWO_D, LBMArray2dHost, LBMArray3dHost>::type;

    Save_HDF5(LBMArrayHost data_h,
        LBMParams& params,
        ConfigMap& configMap,
        const int2str_t& index2names,
        int iStep,
        ...
        , iStep(iStep)
        , totalTime(totalTime)
        , m_times_saved(m_times_saved) {};
    ~Save_HDF5() {};

    /**
     * Copy data_h to buffer data (then transfered to HDF5 write route) - 2d.
     */
        ...
        if (layout == KOKKOS_LAYOUT_RIGHT) { // transpose array to make data contiguous in memory
            for (int j = 0; j < jsize; ++j) {
                for (int i = 0; i < isize; ++i) {
                    int index = i + isize * j;
                    data[index] = data_h(i, j, nvar);
                }
            }
        } else {
            data = data_h.data() + isize * jsize * nvar;
        }

    } // copy_buffer

    /**
     * Copy data_h to buffer data (then transfered to HDF5 write route) - 3d.
     */
    template <DimensionType d_ = d>
            ...
            for (int k = 0; k < ksize; ++k) {
                for (int j = 0; j < jsize; ++j) {
                    for (int i = 0; i < isize; ++i) {
                        ...
                        data[index] = data_h(i, j, k, nvar);
                    }
                }
            }

        } else {
            data = data_h.data() + isize * jsize * ksize * nvar;
        }
        ...
        KokkosLayout& layout)
    {
        ...
        const std::string varName = "/" + index2names.at(varId);
     ...
     *
     * If library HDF5 is not available, do nothing.
     * \param[in] data_h host data to save
     */
    void save()
    {
        ...
        // here we need to check data_h memory layout
        KokkosLayout layout;
        if (std::is_same<typename LBMArray::array_layout, Kokkos::LayoutLeft>::value)
        ...
        std::string baseName;
        if (isFinal) {
            baseName = outputPrefix + "_FINAL";
        } else {
            baseName = outputPrefix + "_" + outNum;
        }
        std::string hdf5Filename = baseName + ".h5";
        std::string hdf5FilenameFull = outputDir + "/" + hdf5Filename;

        // data size actually written on disk - global sizes (accros all MPI processes)
        int nxg = nx;
        int nyg = ny;
        int nzg = nz;
        if (ghostIncluded) {
        ...
        // be careful with hdf5 convention: HDF5 store data along the fastest dimension
        // it means, it assumes a 2d array in C declared like this
        // double data[5][3];
        // the first dimension 5 is the slowest
        // the second dimension 3 is the fastest
        // HDF5 expect data dimension in the right layout
        // so if we actually have a Cuda array (Left layout), we need to swap dimensions
        ...
        for (auto& iter : index2names) {
            // get macroscopic variable index
            ...
            // write variable
            write_field(index, data, file_id, dataspace_memory,
        ...
        // write number of saves done before
        {
            ds_id = H5Screate(H5S_SCALAR);
            attr_id = H5Acreate2(file_id, "number of saves", H5T_NATIVE_INT,
                ds_id,
                H5P_DEFAULT, H5P_DEFAULT);
            status = H5Awrite(attr_id, H5T_NATIVE_INT, &m_times_saved);
            status = H5Sclose(ds_id);
            status = H5Aclose(attr_id);
        }
...
public:
    ...
    Save_HDF5_mpi(LBMArrayHost data_h,
        LBMParams& params,
        ConfigMap& configMap,
        const int2str_t& index2names,
        int iStep,
    ...
    /**
     * Copy data_h to buffer data (then transfered to HDF5 write route) - 2d mpi.
     */
    template <DimensionType d_ = d>
    void copy_buffer(typename std::enable_if<d_ == TWO_D, real_t>::type*& data,
        int isize, int jsize, int ksize, int nvar, KokkosLayout layout)
    ...
    /**
     * Copy data_h to buffer data (then transfered to HDF5 write route) - 3d mpi.
     */
    template <DimensionType d_ = d>
    void copy_buffer(typename std::enable_if<d_ == THREE_D, real_t>::type*& data,
        int isize, int jsize, int ksize, int nvar, KokkosLayout layout)
    ...
    void save()
    {
        ...
        // in case we want external ghost zones
        if (ghostIncluded) {
        ...
        // write number of times saved before
        {
            ds_id = H5Screate(H5S_SCALAR);
            attr_id = H5Acreate2(file_id, "number of saves", H5T_NATIVE_INT,
                ds_id,
 ...
 *
 * data_h is allocated here; if halfResolution is activated, an addition
...
public:
    ...
    /**
     *
     * \param[in,out] data_h A host mirror to a Kokkos::View to LBM simulation
     * \param[in]  params
     * \param[in]  configMap
     * \param[in]  index2names map scalar field name to a string
     *
     */
    Load_HDF5(LBMArrayHost data_h,
        LBMParams& params,
        ConfigMap& configMap,
        const int2str_t& index2names)
        : data_h(data_h)
        ...
        , iStep(0)
        , totalTime(0.0)
        , m_times_saved(0) {}; // end constructor

    virtual ~Load_HDF5() {};

    int get_istep() { return iStep; }
    real_t get_totalTime() { return totalTime; }
    real_t get_times_saved() { return m_times_saved; }
        ...
        if (halfResolution) {
            ...
            for (int j = 0; j < jsize; j++) {
                ...
                for (int i = 0; i < isize; i++) {
                    ...
                    data_h(i, j, nvar) = data[iLow + iL * jLow];
            ...
            if (layout == KOKKOS_LAYOUT_RIGHT) {
                ...
                for (int j = 0; j < jsize; ++j) {
                    for (int i = 0; i < isize; ++i) {
                        int index = i + isize * j;
                        data_h(i, j, nvar) = data[index];
                    } // end for i
                } // end for j
            } else {
                // simple copy
                real_t* tmp = data_h.data() + isize * jsize * nvar;
        ...
        int isize, int jsize, int ksize, int nvar, KokkosLayout layout)
    {
        ...
        if (halfResolution) {
            ...
            for (int k = 0; k < ksize; k++) {
                ...
                for (int j = 0; j < jsize; j++) {
                    ...
                    for (int i = 0; i < isize; i++) {
                        ...
                        data_h(i, j, k, nvar) = data[iLow + iL * jLow + iL * jL * kLow];
            ...
            if (layout == KOKKOS_LAYOUT_RIGHT) {
                ...
                // simple copy
                real_t* tmp = data_h.data() + isize * jsize * ksize * nvar;
     ...
     * into ad buffer called "data" and then copy this buffer to data_h.
     *
        ...
        KokkosLayout& layout)
    {
        ...
        hid_t expectedDataType = (sizeof(real_t) == sizeof(float)) ? H5T_NATIVE_FLOAT : H5T_NATIVE_DOUBLE;
    ...
    virtual void load(std::string filename)
    {
        ...
        bool ghostIncluded = configMap.getBool("output", "ghostIncluded", true);
        ...
        // (we could just use data_h), but since we may need to upscale,
        ...
        for (auto& iter : index2names) {
            ...
            read_field(index, data, file_id, dataspace_memory,
        ...
        // read number of saves
        {
            int times_saved;
            group_id = H5Gopen2(file_id, "/", H5P_DEFAULT);
            attr_id = H5Aopen(group_id, "number of saves", H5P_DEFAULT);
            status = H5Aread(attr_id, H5T_NATIVE_INT, &times_saved);
            status = H5Aclose(attr_id);
            status = H5Gclose(group_id);

            m_times_saved = times_saved;
        }
...
public:
     ...
     *
     */
    Load_HDF5_mpi(LBMArrayHost data_d,
        LBMParams& params,
        ConfigMap& configMap,
        const int2str_t& index2names)
        : Load_HDF5<d>(data_d, params, configMap, index2names) {}; // end constructor
        ...
        KokkosLayout& layout)
    {
        ...
        const std::string varName = "/" + this->index2names.at(varId);
        hid_t dataset_id = H5Dopen2(file_id, varName.c_str(), H5P_DEFAULT);
        ...
        // copy data to data_h array with the correct memory layout
    ...
    void load(std::string filename)
    {
        ...
        const int nbvar = this->index2names.size();
        ...
        // here we need to check data_d / data_h memory layout
        // see https://github.com/kokkos/kokkos/wiki/View - section 6.3.4
        KokkosLayout layout;
        if (std::is_same<typename LBMArray::array_layout, Kokkos::LayoutLeft>::value)
            layout = KOKKOS_LAYOUT_LEFT;
        ...
        // read fields
        for (auto& iter : this->index2names) {
            // get macroscopic variable index
            const int index = iter.first;
        ...
        Kokkos::deep_copy(this->data_d, this->data_h);
